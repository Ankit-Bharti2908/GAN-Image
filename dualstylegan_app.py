...
# Function to run Streamlit locally
def run_streamlit_local():
    """
    Launch the Streamlit app locally (no ngrok)
    """
    ...
    # For Colab, use their built-in port forwarding
    if is_colab:
        from google.colab import output
        print("Starting Streamlit in Google Colab...")
        
        # Start Streamlit directly with run_app function
        cmd = f"""
            import streamlit as st
            from dualstylegan_app import run_app

            if __name__ == "__main__":
                run_app()
            """
        with open("streamlit_runner.py", "w") as f:
            f.write(cmd)
        
        # Run the streamlit command in the background
        subprocess.Popen(["streamlit", "run", "streamlit_runner.py"], 
        ...
        # Use Colab's localtunnel feature
        output.eval_js('google.colab.kernel.proxyPort(8501)')
        
        print("Streamlit is running! Click the link above to access the UI.")
    else:
        # For local environments, just run streamlit normally
        print("Starting Streamlit locally...")
        ...
        # If in a Jupyter notebook, provide a clickable link
        try:
            from IPython.display import display, HTML
            display(HTML('<a href="http://localhost:8501" target="_blank">Click here to open DualStyleGAN UI</a>'))
        except ImportError:
            pass
